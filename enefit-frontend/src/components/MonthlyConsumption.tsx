    ...
    const fetchConsumptionData = async () => {
      ...
      try {
        const response = await axios.get<{ [key: string]: number }>(
          `http://localhost:8080/api/consumptions/monthly/${meteringId}`
        );
        ...
        <table className="w-full text-center border-collapse border border-green-300">
          <thead>
            <tr className="bg-green-200">
              ...
              <th className="border border-green-300 p-2">
          ...
          <tbody>
            {consumptionData.map((data) => (
              <tr key={data.month} className="odd:bg-green-100 even:bg-white">
                <td className="border border-green-300 p-2 font-medium">
                  {data.month}
                </td>
      ...
      <ResponsiveContainer width="100%" height={400}>
        ...
        >
          ...
          <Tooltip
            ...
            formatter={(value) =>
              typeof value === "number" ? `${Math.round(value)} €/MWh` : value
            }
